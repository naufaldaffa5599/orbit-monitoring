/* ================================================================
   ShaderBackground — "Ethereal Whispers" (21st.dev Shader Builder, "Smoke")
   Vanilla-JS port of the original React component. Zero dependencies:
   one WebGL canvas that fills its parent.

     ShaderBackground.mount(document.getElementById("bg-shader"))
       → returns a dispose() function

   The GLSL below is byte-for-byte the upstream shader; only the React
   lifecycle around it (useEffect/useRef) was replaced with a mount/dispose
   pair so the plain-HTML pages here can use it.
   ================================================================ */
...
`;
    ...
    // Tuned for use as a dashboard backdrop: the upstream recipe's near-white
    // highlight is pulled down so panel text stays readable over it, and a
    // vignette darkens the edges the sidebar and topbar sit on.
    const UNIFORMS = {
        colors: [
            [0.043137255, 0.050980392, 0.066666667],
            [0.478431373, 0.517647059, 0.588235294],
            [0.219607843, 0.243137255, 0.298039216],
            [0.082352941, 0.094117647, 0.121568627],
            [0.082352941, 0.094117647, 0.121568627],
            [0.082352941, 0.094117647, 0.121568627],
            [0.082352941, 0.094117647, 0.121568627],
            [0.082352941, 0.094117647, 0.121568627],
    ...
    function mount(canvas) {
        ...
        // No WebGL (old Android WebView, GPU blocklist): the CSS gradient
        // underneath the canvas is the fallback, so just bail quietly.
        if (!gl) {
    ...
    // Auto-mount every canvas that opts in, so pages only need the markup.
    function autoMount() {
        document.querySelectorAll("canvas[data-shader-bg]").forEach(mount);
    }
