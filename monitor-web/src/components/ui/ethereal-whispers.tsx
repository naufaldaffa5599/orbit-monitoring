...
// Tuned for use as a dashboard backdrop rather than a hero: the upstream
// recipe's near-white highlight is pulled down so panel text stays readable
// over it, and a vignette darkens the edges the sidebar and topbar sit on.
const UNIFORMS = {
  colors: [[0.043137254901960784,0.050980392156862744,0.06666666666666667],[0.47843137254901963,0.5176470588235295,0.5882352941176471],[0.2196078431372549,0.24313725490196078,0.2980392156862745],[0.08235294117647059,0.09411764705882353,0.12156862745098039],[0.08235294117647059,0.09411764705882353,0.12156862745098039],[0.08235294117647059,0.09411764705882353,0.12156862745098039],[0.08235294117647059,0.09411764705882353,0.12156862745098039],[0.08235294117647059,0.09411764705882353,0.12156862745098039]] as [number, number, number][],
  colorCount: 4,
  scale: 1.960,
  intensity: 0.750,
  paramA: 0.500,
...
// Phones pay for this shader in a way desktops do not. Every fragment runs
// five fbm evaluations of five octaves each — about a hundred hash calls per
// pixel — so cost is purely a function of how many pixels get shaded, and a
// 3x-DPR phone screen asks for more of them than a laptop does.
//
// Two dials, both only turned down below the layout breakpoint:
//
//   budget — the canvas backing store is capped at this many pixels and CSS
//     upscales it to fit. The field is low-frequency smoke with a scrim over
//     it; at 300k pixels stretched over a phone screen the difference is not
//     visible, and it is roughly a quarter of the work.
//   frame  — timeScale is 0.632, i.e. the image drifts slowly enough that 30fps
//     looks the same as 60 while costing half as much. The rest of the UI still
//     scrolls and animates at the display's own rate; this caps only the redraw
//     of the backdrop.
const HANDHELD_QUERY = "(max-width: 820px)"
const DESKTOP_BUDGET = 2_000_000
const HANDHELD_BUDGET = 300_000
const HANDHELD_MIN_FRAME_MS = 1000 / 30
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Read per-resize rather than captured once: rotating a phone or dragging a
    // desktop window narrow crosses the breakpoint without remounting.
    ...
    const resizeCanvas = () => {
      ...
      const budget = handheld.matches ? HANDHELD_BUDGET : DESKTOP_BUDGET
    ...
    // An arrow const, not the upstream `function render(…)`. A function
    // declaration is hoisted, so TypeScript cannot prove it never runs before
    // the `if (!canvas) return` / `if (!gl) return` guards above and discards
    // their narrowing — under `strict` that makes every canvas/gl use in here
    // an error. Same code, same call order; only the binding form changed.
    const render = (now: number) => {
      raf = 0
      if (disposed || !visible || !inView) return
      // Frame cap: keep the rAF chain alive but skip the draw. lastNow is left
      // alone so the pointer easing still integrates the real elapsed time on
      // the frame that does render.
      if (handheld.matches && now - lastDraw < HANDHELD_MIN_FRAME_MS) {
        requestRender()
        return
      }
      lastDraw = now
